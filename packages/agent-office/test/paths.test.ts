...
describe("path builders", () => {
  // Expected values are built with join() so they use native path separators
  // (POSIX "/" vs Windows "\") and match the builders on every platform.
  it("rolePath nests under office/roles", () => {
    expect(rolePath({ storeRoot: "/s", roleId: "r1" })).toBe(
      join("/s", "office", "roles", "r1.json"),
    );
    expect(rolesDir("/s")).toBe(join("/s", "office", "roles"));
  });
  it("agentPath nests under office/<wk>/agents", () => {
    expect(agentPath({ storeRoot: "/s", workspaceKey: "wk", officeAgentId: "a1" })).toBe(
      join("/s", "office", "wk", "agents", "a1.json"),
    );
    expect(agentsDir("/s", "wk")).toBe(join("/s", "office", "wk", "agents"));
  });
  it("taskPath nests under office/<wk>/tasks/<agent>", () => {
    expect(
      taskPath({ storeRoot: "/s", workspaceKey: "wk", officeAgentId: "a1", officeTaskId: "t1" }),
    ).toBe(join("/s", "office", "wk", "tasks", "a1", "t1.json"));
    expect(tasksDir("/s", "wk", "a1")).toBe(join("/s", "office", "wk", "tasks", "a1"));
  });
