...
describe("evaluateContract", () => {
  it("all evidence in cut -> pass", async () => {
    ...
    const result = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [entry], storeRoot, asOf: NOW });
  ...
  it("memory-entry-ref to absent id -> entry-missing", async () => {
    ...
    const c = contract({ requiredEvidence: [{ kind: "memory-entry-ref", value: "00000000-0000-4000-8000-00000000dead" }] });
    const result = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [entry], storeRoot, asOf: NOW });
  ...
  it("ref to suggested/archival -> entry-not-recallable", async () => {
    ...
    const r1 = await evaluateContract({ contract: c1, projectId: PROJECT_ID, entries: [suggested], storeRoot, asOf: NOW });
    ...
    const r2 = await evaluateContract({ contract: c2, projectId: PROJECT_ID, entries: [archival], storeRoot, asOf: NOW });
  ...
  it("recallable ranked below tiny budget -> ranked-below-budget", async () => {
    ...
    const c = contract({ intent: "first", requiredEvidence: [{ kind: "memory-entry-ref", value: e2.id }], tokenBudget: 10 });
    const result = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [e1, e2], storeRoot, asOf: NOW });
  ...
  it("file-ref and keyword matching with normalization/casing", async () => {
    ...
    const cFile = contract({ intent: "redos guard", requiredEvidence: [{ kind: "file-ref", value: "src/a.ts" }] });
    const rFile = await evaluateContract({ contract: cFile, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
    expect(rFile.pass).toBe(true);

    const cKey = contract({ intent: "redos", requiredEvidence: [{ kind: "keyword", value: "REDOS" }] });
    const rKey = await evaluateContract({ contract: cKey, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
    expect(rKey.pass).toBe(true);

    const cMiss = contract({ intent: "redos", requiredEvidence: [{ kind: "keyword", value: "missing" }] });
    const rMiss = await evaluateContract({ contract: cMiss, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
  ...
  it("determinism: two calls identical and no embed invoked", async () => {
    ...
    const r1 = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
    const r2 = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
  ...
  it("asOf validity: valid at asOf but expired now -> pass", async () => {
    ...
    const c = contract({ intent: "deploy", requiredEvidence: [{ kind: "memory-entry-ref", value: e.id }] });
    const result = await evaluateContract({ contract: c, projectId: PROJECT_ID, entries: [e], storeRoot, asOf: NOW });
    expect(result.pass).toBe(true);
  });
});
