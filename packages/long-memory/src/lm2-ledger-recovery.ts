...
import { parseCanonicalLegacyLm2QuotaLedger } from "./lm2-legacy-quota-ledger.js";
...
export function parseLm2QuotaLedger(raw: Buffer, workspaceKey: string): Lm2QuotaLedger | null {
  ...
  try {
    ...
    const legacy = parseCanonicalLegacyLm2QuotaLedger(source, text);
    return legacy !== null && legacy.workspaceKey === workspaceKey ? legacy : null;
  } catch {
    return null;
  }
}
