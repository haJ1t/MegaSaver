...
import { selectCurrentStateSnapshots } from "./lm1-state.js";
...
function assertEligible(
  ...
  return (
    result.length === evidenceIds.length &&
    result.every(
      (evidence, index) =>
...
export function createLm1RecallService(input: {
  ...
  return {
    async recall(request) {
      ...
      const structuralLeaves = selectCurrentStateSnapshots(snapshots);
      const transitions = records.filter((record) => record.kind === "state_transition");
      const candidates = [...structuralLeaves, ...transitions];
