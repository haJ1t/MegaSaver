import { type ConnectorContext, ConnectorContextSchema } from "@megasaver/connectors-shared";
...
export function assertGenericCliContext(input: unknown, target: ConnectorTarget): ConnectorContext {
  let parsed: ConnectorContext;
  try {
    parsed = GenericCliContextSchema.parse(input);
  } catch (error) {
    throw new GenericCliConnectorError("context_invalid", "Generic CLI context is invalid.", {
      cause: error,
    });
  }
