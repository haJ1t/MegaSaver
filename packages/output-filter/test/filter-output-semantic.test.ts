...
describe("filterOutput semantic file read (T4)", () => {
  it("produces function-aligned excerpts for a .ts file source", async () => {
    const raw = bigTsFile();
  ...
  it("never rewrites a .ts file body through the command-output compressor", async () => {
  ...
  it("keeps structurally similar declarations: dedupe does NOT drop semantic chunks", async () => {
    ...
    const raw = `{\n${entries.join(",\n")}\n}`;
    const fileResult = await filterOutput(
      base(raw, { source: { kind: "file", path: "data.json" } }),
    );
  ...
  it("command source still line-chunks and still dedupes (unchanged path)", async () => {
  ...
  it("unsupported file extension falls back to line chunks", async () => {
    const raw = Array.from({ length: 120 }, (_, i) => `csv,row,${i}`).join("\n");
    const result = await filterOutput(base(raw, { source: { kind: "file", path: "rows.csv" } }));
    expect(result.excerpts.length).toBeGreaterThan(0);
  });
});
