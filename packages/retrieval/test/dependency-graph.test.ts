...
describe("@megasaver/retrieval dependency graph (cycle guard)", () => {
  ...
  it("does not depend on @megasaver/output-filter", () => {
    const deps = Object.keys(packageJson.dependencies ?? {});
    expect(deps).not.toContain("@megasaver/output-filter");
  });
