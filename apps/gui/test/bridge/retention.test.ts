...
describe("retention bridge routes — POST prune", () => {
  let server: TestServer;
  beforeEach(async () => {
    server = await startTestBridge({
      projects: [PROJECT],
  ...
  it("prunes chunk sets older than {days} and returns the refreshed summary", async () => {
    // "now" in the test bridge defaults to a real Date; both fixtures are dated
    // in 2026-05, so a tiny days window prunes everything older than the cutoff.
    const res = await fetch(`${server.baseUrl}/api/sessions/${SESSION.id}/retention/prune`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ days: 1 }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { removed: number; chunkSets: number };
    expect(body.removed).toBe(2);
    expect(body.chunkSets).toBe(0);
  });

  it("rejects a non-integer / non-positive days with 400 validation_failed", async () => {
    const res = await fetch(`${server.baseUrl}/api/sessions/${SESSION.id}/retention/prune`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  it("rejects unknown body fields with 400 validation_failed", async () => {
    const res = await fetch(`${server.baseUrl}/api/sessions/${SESSION.id}/retention/prune`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ days: 30, bogus: true }),
    });
    expect(res.status).toBe(400);
    const body = (await res.json()) as { code: string };
    expect(body.code).toBe("validation_failed");
