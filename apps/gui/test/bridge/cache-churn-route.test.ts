...
function makeCtx(readEvents: () => TokenSaverEvent[]) {
  ...
  return {
    ctx: {
      ...
      sendJson: (_res: unknown, s: number, b: unknown) => { status = s; body = b; },
...
describe("handleGetCacheChurn", () => {
  it("returns live CacheChurnResult", async () => {
    const evt = { id: "a", savingRatio: 0.8, rawBytes: 1000, bytesSaved: 800, returnedBytes: 200 } as unknown as TokenSaverEvent;
    const h = makeCtx(() => [evt]);
