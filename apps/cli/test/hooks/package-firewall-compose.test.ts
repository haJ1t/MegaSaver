...
describe("composeGuardOutputs — the ONE guard-run composition seam", () => {
  it("all none → empty (inert — hook stdout byte-identical to today)", () => {
    expect(composeGuardOutputs({ firewall: none, packageFirewall: pkgNone, meshAdditional: undefined })).toBe("");
  });

  it("firewall warn + package none → byte-identical to today's warn JSON", () => {
    expect(composeGuardOutputs({ firewall: fwWarn, packageFirewall: pkgNone, meshAdditional: undefined })).toBe(
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"FW WARN"}}',
    );
  });

  it("firewall none + package warn → package-only JSON", () => {
    expect(composeGuardOutputs({ firewall: none, packageFirewall: pkgWarn, meshAdditional: undefined })).toBe(
  ...
  it("deny + package warn → deny wire; the package text appears NOWHERE", () => {
    const out = composeGuardOutputs({ firewall: deny, packageFirewall: pkgWarn, meshAdditional: undefined });
    expect(out).toBe(
  ...
  it("firewall warn + package warn → ONE additionalContext with a single-\\n join", () => {
    expect(composeGuardOutputs({ firewall: fwWarn, packageFirewall: pkgWarn, meshAdditional: undefined })).toBe(
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"FW WARN\\nPKG WARN"}}',
    );
  });
  ...
  it("package warn + mesh only → package then \\n\\n mesh (mesh stays outside the seam join)", () => {
    expect(composeGuardOutputs({ firewall: none, packageFirewall: pkgWarn, meshAdditional: "MESH" })).toBe(
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"PKG WARN\\n\\nMESH"}}',
    );
  });

  it("firewall warn + package warn + mesh → fw \\n pkg \\n\\n mesh", () => {
    expect(composeGuardOutputs({ firewall: fwWarn, packageFirewall: pkgWarn, meshAdditional: "MESH" })).toBe(
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"FW WARN\\nPKG WARN\\n\\nMESH"}}',
    );
  });

  it("deny + package warn + mesh → deny wire carrying mesh (today's wire); package nowhere", () => {
    const out = composeGuardOutputs({ firewall: deny, packageFirewall: pkgWarn, meshAdditional: "MESH" });
    expect(out).toBe(
  ...
  it("mesh-only (project null / no match) → today's mesh-only wire", () => {
    expect(composeGuardOutputs({ firewall: none, packageFirewall: pkgNone, meshAdditional: "MESH" })).toBe(
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"MESH"}}',
    );
  });
});
