...
describe("standalone CLI bundle", () => {
  ...
  it.skipIf(!runBundleCases)("recognizes all top-level Wave-2 commands in built mega.mjs", () => {
    const isolatedRoot = mkdtempSync(join(tmpdir(), "megasaver-bundle-wave2-"));
    try {
      const fenceOut = execFileSync(process.execPath, [bundle, "fence", "status"], {
        encoding: "utf8",
        env: isolatedBundleEnv(isolatedRoot),
      });
      expect(fenceOut).toContain("fence is disabled");

      const budgetOut = execFileSync(process.execPath, [bundle, "budget", "status"], {
        encoding: "utf8",
        env: isolatedBundleEnv(isolatedRoot),
      });
      expect(budgetOut).toContain("token budget");

      const upOut = execFileSync(process.execPath, [bundle, "up", "--plan", "--json"], {
        encoding: "utf8",
        cwd: isolatedRoot,
        env: isolatedBundleEnv(isolatedRoot),
      });
      ...
      let reviewPackOut = "";
      try {
        reviewPackOut = execFileSync(process.execPath, [bundle, "review", "pack", "--json"], {
          encoding: "utf8",
          cwd: isolatedRoot,
          env: isolatedBundleEnv(isolatedRoot),
        });
