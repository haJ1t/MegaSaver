...
function evt(id: string, savingRatio: number): TokenSaverEvent {
  return { id, sessionId: "s1", workspaceKey: "w1", createdAt: "2026-08-10T10:00:00.000Z", sourceKind: "Bash", label: "rg", rawBytes: 1000, returnedBytes: 200, bytesSaved: 800, savingRatio, summary: "x" } as unknown as TokenSaverEvent;
}
...
describe("mega cache-doctor", () => {
  ...
  it("empty store --json returns zero rate", async () => {
    ...
    const lines: string[] = [];
    await runCacheDoctor({ storeRoot: store, json: true, readEvents: () => [], stdout: (l) => lines.push(l), stderr: () => {} });
    expect(JSON.parse(lines.join(""))).toMatchObject({ cacheInvalidationRate: 0, recommendation: "keep_enabled" });
    rmSync(store, { recursive: true, force: true });
  });
  ...
  it("human table contains recommendation", async () => {
    ...
    const lines: string[] = [];
    await runCacheDoctor({ storeRoot: store, json: false, readEvents: () => [evt("a", 0.8)], stdout: (l) => lines.push(l), stderr: () => {} });
