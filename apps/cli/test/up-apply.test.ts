...
import { runUpApply, type UpApplyDeps } from "../src/up/apply.js";
...
describe("runUpApply", () => {
  ...
  it("executes install steps in order and writes manifest with priors", async () => {
    ...
    const deps: UpApplyDeps = {
      ...
      connectorSync: vi.fn(async (_name) => {
        calls.push("connectorSync");
        return 0;
      }),
      saverEnable: vi.fn(async () => {
  ...
  it("stops on first failure and keeps completed steps in manifest", async () => {
    ...
    const deps: UpApplyDeps = {
      hooksInstall: vi.fn(() => 0),
      ensureProject: vi.fn(async () => ({ code: 0, name: "demo", created: true })),
      connectorSync: vi.fn(async (_name) => 0),
      saverEnable: vi.fn(async () => 1), // fail saverEnable
      now: () => "2026-08-06T10:00:00.000Z",
    };
  ...
  it("skips ok steps and performs zero dep calls when all ok", async () => {
    ...
    const deps: UpApplyDeps = {
      hooksInstall: vi.fn(() => 0),
      ensureProject: vi.fn(async () => ({ code: 0, name: "demo", created: false })),
      connectorSync: vi.fn(async (_name) => 0),
      saverEnable: vi.fn(async () => 0),
      now: () => "2026-08-06T10:00:00.000Z",
    };
