...
export function buildAuditionReport(fixtures: AuditionReport["fixtures"]): AuditionReport {
  return {
    version: 1,
    fixtures,
    verdict: "On this fixture, delivery was smaller than raw. This is a byte counter, not a bill claim.",
  };
}
...
export function renderAuditionReport(report: AuditionReport): string {
  ...
  for (const f of report.fixtures) {
    lines.push(`${f.name}: ${f.rawBytes} -> ${f.deliveredBytes} bytes, chunks=${f.chunks}, exit=${f.exitCode}`);
  }
