...
- **Build-order preconditions (7 of 20, wave-2 batch).** This plan CONSUMES and never redefines: `childExitCode` on both event schemas + the exec writers (claim-verification-gate plan Task 1, `docs/superpowers/plans/2026-08-06-claim-verification-gate.md:28`) and its Stop plumbing — `hasStopHook`/`addStopHook`/`removeStopHook`/`writeSettingsFile` exports (gate Task 7, `:1222`); `listOverlayChunkSets` + `CAPSULE_FILENAME` (compaction-guard plan Task 1, `docs/superpowers/plans/2026-08-06-compaction-guard.md:23` — cross-pair single definition) and `workStateCapsuleSchema` in `apps/cli/src/hooks/capsule.ts` (compaction-guard Task 3, `:275`). If any is unlanded when a task here needs it, land that owning task first; do not re-implement. At RUNTIME absence is data, not a crash: zero `childExitCode` rows or a missing capsule → the dependent detector degrades to `no-signal` (spec Dependencies).
...
import type { ChunkSetSummary } from "@megasaver/content-store"; // packages/content-store/src/chunk-set.ts:57
import type { OverlayTokenSaverEvent, ReadIndexEntry } from "@megasaver/core";
import type { WorkStateCapsule } from "../../hooks/capsule.js"; // compaction-guard Task 3
...
- **context-overflow:** `refs === undefined` → `no-signal` ("no input text — pass --file or pipe stdin"). ASSUMPTION (verdict composition): known ids = `chunkSets[].chunkSetId` ∪ defined `events[].chunkSetId`; every `chunkRefs` entry not in the set is a dangling-ref finding ("referenced chunk <id> is not in this session's store"); input present with zero dangling (including zero chunk refs) → `clear`; `capsule !== undefined` only annotates the `fix` line ("a compaction capsule exists — referenced content may predate compaction; re-fetch with mega output chunk"), it never creates a finding on its own.
- **hallucinated-state (3-way, Decision 8; only `phantom` is a finding):** `refs === undefined` → `no-signal`. ASSUMPTION: `readIndex === undefined` AND `chunkSets.length === 0` → `no-signal` ("no capture stores for this session"). Otherwise per path ref: resolve against `cwd`; outside-workspace (via `path.relative` escaping) → `info`, NEVER probed; inside: `captured` iff `readIndex?.[hashPath(abs)]` exists OR some chunk-set has `source.kind === "file"` with `source.path === redactText(abs)` (chunk-set `source` union: `packages/content-store/src/chunk-set.ts:22`; the redact-string match may miss a secret-bearing path — accepted spec open question; the `hashPath` leg is redaction-free); not captured and `fileExists(abs)` → `exists-uncaptured` `info` (the saver captures only Read/LS/Bash/Grep/Glob/WebFetch — `apps/cli/src/hooks/saver.ts:28` — so agent-written files legitimately miss the index); neither → `phantom` finding. Existence probes are metadata-only and confined to cwd-contained paths.
  ...
  it("no input → no-signal; dangling chunk ref → finding; resolving ref → clear", () => {
    ...
    const resolved = run(
      snap({
        ...
        chunkSets: [{ chunkSetId: "cs-aaaaaaaaaaaa", createdAt: "2026-08-06T11:00:00.000Z", source: { kind: "file", path: "src/a.ts" }, rawBytes: 5, redacted: true, chunkCount: 1 }],
      }),
    );
