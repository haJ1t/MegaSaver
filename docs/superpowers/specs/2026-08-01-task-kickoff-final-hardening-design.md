...
On POSIX slash-absolute paths, only the launchers Mega Saver can actually emit
there (`mega`, `mega.mjs`, and approved development `cli.js`) are owned;
Windows-only `.cmd` and `.exe` suffixes require a recognized drive or UNC path.
...
- a POSIX absolute `.cmd` or `.exe` launcher remains foreign and survives
  reinstall/uninstall;
