...
- `specs/index.html` — the spec library (`/specs`): a searchable, risk-filterable
  catalog of all 288 design specs in `docs/superpowers/specs/`, each linked to
  its source file in the repository. Generated by `scripts/gen-specs-page.mjs`
  (frontmatter title + a purpose extract from the Goal / Problem / TL;DR section).
  Regenerate when specs change:

  ```bash
  node scripts/gen-specs-page.mjs
  ```

- `harnesses/index.html` — the supported-harnesses page (`/harnesses`): the
  detection catalog from `@megasaver/harness-detect`, grouped by category with
  per-harness detection signals and integration level. Generated by
  `scripts/gen-harnesses-page.mjs` from the built package dists (single source
  of truth). Regenerate when the catalog changes:

  ```bash
  pnpm --filter @megasaver/harness-detect build     && pnpm --filter @megasaver/connector-generic-cli build     && node scripts/gen-harnesses-page.mjs
  ```
