...
- [[entities/memory-recall]] — `@megasaver/memory-recall` read-only adapter that ranks Core memories through LM2 Safe/Adaptive recall with current-sidecar-hash checks.
...
- [[entities/hot-handoff]] — i10: `mega handoff pack/open/inspect/clear` — redacted, expiring `.megahandoff` task packet carries live task state (brief + recallable memories + unresolved failures + secret-path-filtered dirty diff) between agents mid-session; suggested-gate memory merge; `hot-handoff` ProFeature (dry-run/inspect/clear free); reuses bundle-frame/warm-start/connectors-shared/policy. Branch `worktree-feat-hot-handoff`, pending merge.
...
- [[sources/cache-write-reduction-design]] — approved-direction design for cache-write reduction: stable task kickoff, batch-read advice, cache suffix audit, and safe output-route advice; all claims are benchmark-gated.
...
- [[syntheses/verify-fresh-audit-2026-08-01]] — **superseded 2026-08-05, defect fixed.** Found that `TOKEN_COUNT_BUDGET_MS` cannot fire because the encode is synchronous — correct, and the basis for `2026-08-05-saver-token-count-bound-design.md`. Its "verify is RED" claim and its repeated-character root cause are both stale: cost is quadratic in each regex match's UTF-8 bytes, so 32 KB of newlines took 46 s with no character repeated.
...
- `archive/mega-agent.md` — **abandoned** (2026-08-24): the Rust agent-harness plan was dropped entirely; branch deleted, spec/plan/review docs removed.
- `archive/first-party-agent-mission-change.md` — **reverted** (2026-08-24): recorded the first-party-agent mission inversion; abandoned with the harness plan.
- `archive/supervisor-agent-split.md` — **abandoned** (2026-08-24): harness supervisor/agent sandbox split decision; no code ever merged.
- `archive/conductor-is-a-role.md` — **moot** (2026-08-24): resolved a contradiction inside the abandoned harness spec; mesh's own non-goal stands alone.
...
- **v1.0.0 — FIRST STABLE PRODUCT RELEASE** — TAGGED + PUBLISHED 2026-08-24 (`feab158e`, PR #365): all 17 accumulated changesets versioned in one stable cut; `@megasaver/cli@1.0.0` live on npm (`latest`, zero deprecation flags) and GitHub Release `v1.0.0` with the standalone bundle (`mega.mjs`). All prior GitHub releases/tags deleted at operator request; **npm legacy sweep fully DONE** — all 23 pre-reset versions carry the upgrade-to-1.0.0 deprecation notice, and the stray flag that landed on 1.0.0 itself was cleared. Locked sub-decisions + May-"v1.0" disambiguation: [[decisions/v100-stable-reset]].
