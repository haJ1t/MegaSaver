...
reduction on eligible MegaSaver-mediated context.
...
1. Context Gateway observes or proxies read/search/command output.
2. Redaction and compression run before evidence is persisted or returned.
3. An immutable Evidence Ledger stores redacted, expandable evidence references.
4. Agent `save_memory` creates a candidate, not approved memory.
...
The 90% target applies to eligible MegaSaver-mediated large text outputs:
`returnedTokens <= 0.10 * rawTokens` on a rolling average. Small passthrough
outputs and native truncation are reported separately so MegaSaver does not claim
fake savings.
...
- Every committed memory can explain its evidence and policy version.
