...
## [2026-08-15] feat | claim-verification-gate (v2.8 #1) — merged PR #355

C3 of the v2.8 trust slice. `childExitCode` (additive-optional int|null) on both token-saver event schemas, written by both exec orchestrators; `mega verify claims` (6 locked linear-time claim patterns, ReDoS guard revert-proven — the lazy-quantifier revert HUNG the guard corpus; 8 MiB cap enforced at the boundary via stat/stream-limit; 30-min window join; verdicts verified|exit-mismatch|exit-unrecorded|no-receipt; --strict CI gate; --json); opt-in Stop-hook receipt reminder (`mega verify enable-hook/disable-hook`, warn-only, fail-open; removed by full `mega hooks uninstall`; `stopInstalled` in status). code-reviewer APPROVE-WITH-CHANGES → P2s closed (boundary cap, uninstall/status wiring, reminder copy interpolation, --file coverage). CI ubuntu+windows green; squash-merged.

## [2026-08-15] feat | silent-failure-monitor (v2.8 #2) — spec refresh + TDD build

Branch `feat/silent-failure-monitor` (worktree). v2.8 trust slice second pick.

- **Spec+plan refresh (user-approved 2026-08-15):** compaction-guard surfaces (`listOverlayChunkSets`, `CAPSULE_FILENAME`, `workStateCapsuleSchema`) are UNSHIPPED — Decision 8 + Dependencies amended: v1 hardcodes `chunkSets: []` / `capsule: undefined` BY CONSTRUCTION; the chunk-set source capture leg is deferred; the read-index leg carries hallucinated-state in full. Landing order cannot make the monitor lie; additive re-enable when compaction-guard lands.
- **TDD build:** M6 core re-exports `loadReadIndex`/`hashPath`/`ReadIndexEntry`; `scanRefs` (chunk refs = one linear `cs-[0-9a-f]{8,64}` pattern; paths = tokenizer + per-token anchored validators; 8 MiB cap; guard revert-proven 16.47x at 64→256 KiB, threshold 8); snapshot loader (newest-session pick data-derived from `createdAt`, never mtime; every store read degrades); four detectors with closed `DetectorId` union + redact-on-echo centralized; `runAlertsFailures` mode-flag branch BEFORE the Pro entitlement gate (`--days` conflict = usage error); opt-in `failure-scan` Stop hook with a trigger DISJOINT from the gate's reminder (gate: zero receipts; monitor: ≥1 unresolved failing — mutually exclusive by construction), second Stop entry keyed by subcommand.
- **Review P2s closed:** corrupt `read-index.json` reads as undefined → no-signal, never a confident empty index (phantom-finding honesty gap); `unresolvedFailingReceipts` O(n) with precomputed timestamps + success/expansion resolution indexes (was O(m·n) with Date.parse per row on the Stop hot path); store reads in the snapshot fully guarded (EACCES degrades, never throws); cap constant parametrized; toggle honors `--json`.
- Smoke: empty `--json` always-JSON exit 0; `--days` conflict exit 1; seeded failing receipt → `[tool-error] pnpm test — exit 2` + unacknowledged-failure candidate; `--strict` exit 1; opt-outs exit 0; hook Stop envelope warn-only exit 0; toggle writes/strips the failure-scan Stop entry. pnpm verify 62/62 (cli 216 files / 2099 tests).
