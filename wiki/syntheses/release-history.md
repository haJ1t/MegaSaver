...
v1.1.0, v1.0, AA1 BB1–BB7a, v0.3, v0.2 and all their PR-level detail) is
preserved below exactly as it stood in `index.md`. The [[index]] Status
section now carries a one-line-per-release summary pointing here.

For the append-only operational timeline (per-op log entries) see
